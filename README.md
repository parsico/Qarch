# X-UI Pro VERSION / PARSICO Edition
...
# PARSICO AUTO FULL INSTALLER (Change your domain name)

````
apt update && apt upgrade -y && apt install curl socat -y && curl https://get.acme.sh | sh && ~/.acme.sh/acme.sh --set-default-ca --server letsencrypt && ~/.acme.sh/acme.sh --register-account -m ssl@parsico.org && ~/.acme.sh/acme.sh --issue -d  YOURDOMAINNAME --standalone && ~/.acme.sh/acme.sh --installcert -d  YOURDOMAINNAME --key-file /root/private.key --fullchain-file /root/cert.crt && bash <(curl -Ls https://raw.githubusercontent.com/parsico/x-ui-pro/master/install.sh)
````

# Single Command Install & upgrade

````
bash <(curl -Ls https://raw.githubusercontent.com/parsico/x-ui-pro/master/install.sh)
````
